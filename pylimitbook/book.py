...
class Book(object):
    ...
    def process_bid_ask(self, tick):
        ...
        #3:Cancel
        #4:Modify
        if tick.activity_typ == 3:
            tree.remove_order_by_id(tick.id_num)
        elif tick.activity_typ == 4:
            tree.update_order(tick)
        else:
            tree.insert_tick(tick)

    def bid(self, csv):
        columns = ['Record_ind', 'segment', 'order_no', 'quote_tm', 'buysell', 'activity_typ', 'symbol', 'series', 'vol_disc', 'vol_orgnl', 'limit_prc', 'price_trig', 'mkt_ord_flg', 'StLoss_I', 'Io_flag', 'Algo_Ind']
